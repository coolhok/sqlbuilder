...
func NewStruct(structValue interface{}, flavor ...interface{}) *Struct {
	...
	if len(flavor) > 0 {
		...
		switch iter.(type) {
		case Flavor:
			s.Flavor = iter.(Flavor)
		default:
			s.Flavor = DefaultFlavor
		}
	}
...
func (s *Struct) parse(t reflect.Type) {
	...
	for i := 0; i < l; i++ {
		...
		// Parse from db key word
		if s.Flavor == DefaultFlavor {
			if _, ok := mysqlkeyword[strings.ToUpper(alias)]; ok {
